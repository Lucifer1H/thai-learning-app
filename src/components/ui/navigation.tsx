      ...
      <div className="w-full px-6 lg:px-12 xl:px-16">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-3 flex-shrink-0">
            <Link
              href="/profile"
              className="flex items-center space-x-2 text-gray-700 hover:text-blue-600 transition-colors px-2 py-1 rounded-md"
            >
              <User className="h-5 w-5" />
              <span className="text-sm chinese-text">
                {user?.user_metadata?.full_name || user?.email?.split('@')[0] || '1'}
              </span>
            </Link>
            <button
              onClick={handleSignOut}
              className="flex items-center space-x-1 text-gray-500 hover:text-gray-700 transition-colors px-2 py-1 rounded-md"
            >
